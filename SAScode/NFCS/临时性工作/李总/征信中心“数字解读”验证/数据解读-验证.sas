...
%do i = 1 %to &n.;
	...
	;
	run;
PROC EXPORT DATA=&dataset._&&yuefen&i. OUTFILE="E:\新建文件夹\SAS\常用代码\自动化\结果文件夹\临时性工作\李总\数据解读\shzx_app_&&yuefen&i." DBMS=TAB;
DELIMITER=',';
RUN;
...
%do i = 1 %to &n.;
	...
	;
	run;
PROC EXPORT DATA=&dataset._&&yuefen&i. OUTFILE="E:\新建文件夹\SAS\常用代码\自动化\结果文件夹\临时性工作\李总\数据解读\shzx_accm_&&yuefen&i." DBMS=TAB;
DELIMITER=',';
RUN;
proc delete lib=work data = &dataset._&&yuefen&i.; 
run;
...
;
quit;
libname xls excel "E:\新建文件夹\201509\数字解读验证\验证数据提取单-附件.xlsx";
